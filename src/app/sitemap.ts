...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // `/join` is intentionally absent: it renders `robots: noindex` by design.
  return [
    ...pages,
    ...
    ...galleries.map((g) => ({
      ...
      lastModified: at(g.createdAt),
      priority: 0.5,
      // Feed Google Images the first few frames of each photo story.
      images: g.photos.slice(0, 4).map((p) => absoluteUrl(p.url)),
    })),
    ...news.map((n) => ({ url: `${base}/news/${n.id}`, lastModified: at(n.date), priority: 0.5, images: n.imageUrl ? [absoluteUrl(n.imageUrl)] : undefined })),
