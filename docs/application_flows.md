...
*   **User Story**: An admin needs to write a news article but only has a few bullet points from a press release or match notes. They also need to upload a header image for the article.
*   **UI (`NewsEditor.tsx`)**: The admin enters bullet points into a `Textarea` and uses a file input to select an image.
...
*   **Backend (`news.ts`)**: The `addNewsArticle` function saves the final headline, content, uploaded image, and any associated data to the `news` collection in Firestore.
